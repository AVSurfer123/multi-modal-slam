...
void velo16_handler( const sensor_msgs::PointCloud2::ConstPtr &msg )
{
    ...
    const float time_unit_scale = 1.0; // Corresponds to defaults of milliseconds
    bool given_offset_time = false;

    pcl::PointCloud<velodyne_pcl::PointXYZIRT> pl_orig;
    ...
    for ( int i = 0; i < pl_orig.points.size(); i++ )
    {
        ...
        if ( 0 ) // For debug
        {
            if ( pl_processed.size() % 1000 == 0 )
            {
                ...
                cout << ( int ) ( pl_orig.points[ i ].ring ) << ", " << ( pl_orig.points[ i ].time / 1e9 ) << ", " << range << endl;
            }
        }
    }
